...
export default function HomeClient({ recipes }: { recipes: Recipe[] }) {
  ...
  const videoID = useMemo(() => (featured?.LinkYT ? getYoutubeVideoID(featured.LinkYT) : null), [featured]);
  const thumb = featured?.Image || (videoID ? `https://img.youtube.com/vi/${videoID}/hqdefault.jpg` : null);
